...
public interface IJdbcBaseDao {
	 ...
	 * 结果集row属性转换为bean属性
	 * @param sql 静态sql
	 * @param beanClazz bean对象
	 * @param extractor 结果集抽取器，会将rs属性提取为bean属性，并返回存储bean对象的list
	...
	/**
	 * 使用RowMapper类来映射结果集中的每行记录到javabean中
	 * 不支持自定义结果集行提取器,使用默认结果集行提取器来解析
	 * @param sql 静态sql
	 ...
	 * @param sql 静态sql
	 * @param rowMapper 自定义的rowmapper实现类
	 * @return 存储javabean的list对象
	 * @throws DaoException
	 */
