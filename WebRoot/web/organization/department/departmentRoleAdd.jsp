		        ...
		        <table width="100%" border="0" cellspacing="0" cellpadding="0">
		          ...
		          <tr>
		            ...
		            <td width="10%">
		            	<!-- 搜索div开始 -->
						<div>
						<form name="departmentRoleAddForm" method="post" action="">
						<input type="hidden" id="currentPage" name="currentPage" value="1" />
						<input type="hidden" id="ids" name="role.role.id" value="" />
						<input type="hidden" id="departmentID" name="id" value="<%=request.getParameter("id") %>" />
									...
									<span class="shell_tool_search textInput"> 
										<span>
										 <input	type="text" class="inputtext DOMControl_placeholder"
											...
											<button type="button" title="Search for documentation" onclick="javascript:doDepartmentUnbindRoleSearch();"></button> 
										</span>
									</span>
								</div>
							</div>
							...
							%>	
	                         ...
	                         <tr onclick="doChangeColor('color<%=j %>');" id="color<%=j %>" bgcolor="#FFFFFF">
	                          <td width="5%" align=center  class="a1">
	                          	<input type="checkbox" name="checkedUser" value="<%=role.getId() %>" 
...
<script type="text/javascript">
	...
	function doAction(action){
		if(action == 'ok'){
			...
			if(selectedCount<=0){
				...
				if(window.confirm("确定分配角色给该部门？")){
					//循环取出选择对象id值，以“-”进行分隔
					getcheckBoxAllValue("checkedUser","ids");
					doSubmit(null,"<%=request.getContextPath() %>/web/organization/department/assignSysRole.action","departmentRoleAddForm");
	...
	//---------------------- 未绑定角色搜索 -------------------------------------------------------------------------
	function doDepartmentUnbindRoleSearch(){
		var _action = "<%=request.getContextPath() %>/web/organization/department/unAssignRoleIndex.action?currentDepartmentID=" + document.getElementById("departmentID").value;
		doSubmit(null,_action,"departmentRoleAddForm");	
		window.returnValue = 'ok';
	}
	...
	function doPaging(_currentPage){
		var _action = "<%=request.getContextPath() %>/web/organization/department/unAssignRoleIndex.action?currentDepartmentID=" + document.getElementById("departmentID").value;
		doSubmit(_currentPage,_action,"departmentRoleAddForm");
		window.returnValue = 'ok';
	}
